...
2. Scan: `AGENTS.md`, `.cursor/skills/`, `.agent/`, `docs/`, git remotes, lockfiles (`Pipfile`, `package.json`, `go.mod`, `pyproject.toml`).
...
Do not add unused integrations. Review bots such as Revy only when the user says they run on this repo's PRs.

Write `.agent/flows/*.json` from pack templates for **installed** flows only.
